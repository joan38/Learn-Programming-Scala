...
```
You can notice that it displays the name and the infered type but **not the result** as it's not evaluated yet.  
...
* **actions** are represented by functions (jump, greet, priceTaxIncluded may be confusing though should be understood as calculatePriceTaxIncluded...).

Moreover, we prefer to name functions by verbs and values by names to distinguish easily a property from an action.

## Parameters

Now that you know how to declare a function, remember what said your Maths teacher far far long time ago about what is a function. What! you where not listening?  
...
scala> def course(title: String => String) =
     ...
     |     "Now we will learn Higher Order Functions"
course: (s: String, title: String => String)String
```
* The type `String => String` is a **function** that takes a `String` and return a `String`.
* We can call the given function `title` just like other functions.
